import CONST #for categories_data
...
def verify_mods(mods, data):
	...
	def convert_category_number(nexus_categoryN):
			...
			"""
			if Game == 'Fallout 4':
				categories_data = CONST.fallout4_default_categories_dat
			elif Game == 'Skyrim':
				categories_data = CONST.skyrim_default_categories_dat
	...
	for mod in mods:
		...
		if data.get(mod_file_name) is not None:
			mod_checksum = m2modpack_tools.make_checksum(mod)
...
def main(Game, modpack_json, mods_repo, MO_bin, debug=debug):
	...
	#------------------------------get file list------------------------------
	mods_list = m2modpack_tools.scan_dir(mods_repo)
	#------------------------------- load json ---------------------------------
	mods_data = m2modpack_tools.try_load_json(modpack_json)
	#------------------------------- verify json ---------------------------------
